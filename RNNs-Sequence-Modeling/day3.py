...
lst_model.build(input_shape=(None, max_len))  # None = batch size, max_len = sequence length

lst_model.summary()

history = lst_model.fit(X_train, y_train, epochs=5, batch_size=32, validation_split=0.2)
loss, accuracy = lst_model.evaluate(X_test, y_test)
